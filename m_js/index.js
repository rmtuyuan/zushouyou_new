...
var index = {
    ...
    bannerLoad: function () {
        //轮播图
        fetch("http://192.168.3.118/Api/Index/flash", {
            method: 'POST',
            ...
            if (data.code == 200) {
                for (var i = 0; i < data.data.length; i++) {
                    ...
                    banner_html += `<li><a href="https://${element.url}?id=${i}"><img src="http://192.168.3.118${element.pic}" alt="${element.title}"></a></li>`;
                }
    ...
    noticeLoad: function () {
        // 公告
        fetch("http://192.168.3.118/Api/Index/tranlate", {
            method: 'POST',
            //headers:{"Content-type": "application/json; charset=UTF-8" },
            mode: 'cors'
            // cache: 'no-cache',
    ...
    latelyLoad: function () {
        //最近使用
        fetch("http://192.168.3.118/Api/Index/lately", {
            method: 'POST',
            //headers:{"Content-type": "application/json; charset=UTF-8" },
            mode: 'cors'
            // cache: 'no-cache',
            ...
            if (data.code == 200) {
                ...
                for (var i = 0; i < data.data.length; i++) {
                    var element = data.data[i];
                    lately_html += `<div>
                            <a href="gameList_next.html?gameid=${element.id}&shop=${element.shop_id}" class=""><img src="http://192.168.3.118${element.fimg}" alt=""/></a>
    ...
    hotLoad: function () {
        //热门游戏
        fetch("http://192.168.3.118/Api/Index/hot", {
            method: 'POST',
            //headers:{"Content-type": "application/json; charset=UTF-8" },
            mode: 'cors'
            // cache: 'no-cache',
    ...
    phoneLoad: function () {
        //手机游戏
        fetch("http://192.168.3.118/Api/Index/mobile", {
            method: 'POST',
            //headers:{"Content-type": "application/json; charset=UTF-8" },
            mode: 'cors'
            // cache: 'no-cache',
    ...
    pcLoad: function () {
        //pc游戏
        fetch("http://192.168.3.118/Api/Index/computer", {
            method: 'POST',
            //headers:{"Content-type": "application/json; charset=UTF-8" },
            mode: 'cors'
            // cache: 'no-cache',
