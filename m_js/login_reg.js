...
var login = {
    ...
    loginFun: function () {
        ...
        $(".login-btn-box").on("click", "#login_btn.active", function (e) {
            e.preventDefault();
            var form = new FormData();


            form.append("phone", $("#login_phone").val()); // 文件对象
            form.append("password", $(".login-pwd").val()); // 文件对象
            fetch("http://192.168.3.118/Api/index/login", {
                method: 'POST',
                ...
                if (data.code == 200) {
                    //alert("登录成功")
                    //设置cookie                    
                    loginName = data.data.id;
                    $.cookie(cookie_name, loginName, {
                        expires: 7
                    });
                    window.location.href = "http://192.168.3.120/zushouyou_new/index.html";
                } else {
                    $(".login-pwd").siblings(".info-box").show().find("span").html(data.message);
                }
...
var regist = {
    ...
    regSucc: function () {
        if (this.phone_reg_ok && this.pwd_reg_ok && this.pin_reg_ok && this.agmeent_reg_ok) {
    ...
    regGetPin: function () {
        ...
        $(".my-form-group").on("click", ".get-pin.active", function (e) {
            ...
            if (regist.phone_reg_ok) {
                ...
                fetch("http://192.168.3.118/Api/Index/yzm1", {
                    method: 'POST',
                    //headers:{"Content-type": "application/json; charset=UTF-8" },
                    mode: 'cors',
                    // cache: 'no-cache',
    ...
    chkFun: function () {
        ...
        $("#read_agreement_chk").change(function () {
            if ($(this).is(":checked")) {
                ...
                regist.agmeent_reg_ok = true;
            } else {
                $(this).siblings("span").removeClass("active");
                regist.agmeent_reg_ok = false;
            }
            regist.regSucc();
        });
    },
    regBtnClick: function () {
        ...
        $(".form-login").on("click", "#reg_btn.active", function (e) {
            ...
            if (regist.phone_val == $("#reg_phone").val()) {
                ...
                fetch("http://192.168.3.118/Api/index/register", {
                    method: 'POST',
                    //headers: myHeaders,
                    mode: 'cors',
                    cache: 'default',
