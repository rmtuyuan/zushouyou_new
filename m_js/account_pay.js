...
var accountPay = {
    ...
    dataLoad: function () {
        ...
        fetch("http://192.168.3.118/Api/Index/zy_pay", {
            method: 'POST',
            //headers: myHeaders,
            mode: 'cors',
            cache: 'default',
    ...
    clickBtn: function () {
        ...
        $(".now-pay-btn").click(function (e) {
            ...
            if ($(".pay-way-item input:checked").hasClass("pay-ye")) {
                ...
                if (ua.match(/MicroMessenger/i) == "micromessenger") {
                    ...
                    fetch("http://192.168.3.118/Api/Index/phone_order", {
                        ...
                        if (data.code == 200) {
                            ...
                            window.location.href = `http://192.168.3.118/Vendor/wappay/pay.php?ordername=${data.data.receive_name}&total_fee=${data.data.price}&attach=${data.data.subject}&trade_no=${data.data.orderid}`;
                        } else {
                            alert("支付宝支付失败");
                        }
                    });
                ...
                form.append("pay", pay); //   
                form.append("commodity_id", accountPay.commodity_id); // 
                fetch("http://192.168.3.118/Api/Index/wxpay", {
                    method: 'POST',
                    //headers: myHeaders,
                    mode: 'cors',
                    cache: 'default',
                    ...
                    if (data.code == 200) {
                        alert("微信支付成功");
                           window.location.href = `http://192.168.3.118/Wxpay/example/jsapi.php?ordername=${data.data.receive_name}&total_fee=${data.data.price}&attach=${data.data.subject}&bianhao=${data.data.orderid}`;
                    } else {
                        alert("微信支付失败");
                    }
                });
